...
const Reference = () => {
  ...
  const removeLink = async (index: string) => {
    try {
      setIsLoading(true);
      const req = await fetch(`/api/reference?id=${index}`, {
        method: "DELETE",
      });
      if (req.status === 200) setLinks((prev) => prev.filter((link) => link.id !== index));
    } catch (error) {
      console.error(error);
    } finally {
      setIsLoading(false);
  ...
  return (
    <div className="flex flex-col items-center p-5">
        ...
        {links.map((link, index) => {
          return (
            <div className="relative" key={`web-ref-${link?.url}-${index}`}>
              ...
              ></Input>
              <DeleteAlert referenceId={link.id} removeFn={removeLink}>
                <Image
      ...
      <Button
        onClick={() => setLinks((prev) => [...prev, { url: "", exists: false }])}
        className=" flex p-10 items-center justify-center w-[80vw] rounded-lg bg-[#d9d9d9]"
      >
        <Image width={30} height={30} alt="button-back" src="addButton.svg"></Image>
      </Button>
      {links && (
        <Button
          onClick={() => saveLinks()}
